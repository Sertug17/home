...
describe("durable money action claims", () => {
  test("atomically grants one dispatch and makes every refresh recover the same attempt", async () => {
  ...
  test("scopes reads, claims, and submission references to the verified owner tuple", async () => {
    const store = new MemoryMoneyActionStore();
    await store.issue(action());
  ...
  test("keeps one immutable submission handle and never authorizes a conflicting resend", async () => {
    const store = new MemoryMoneyActionStore();
    await store.issue(action());
  ...
  test("fails closed before dispatch when a sensitive payload overlay is unavailable", async () => {
    const store = new MemoryMoneyActionStore();
    const sensitive = {
      ...action(),
  ...
  test("allows verified read reconciliation from unknown while keeping client terminal writes source-bound", async () => {
    const store = new MemoryMoneyActionStore();
    await store.issue(action());
    await store.claim(OWNER, action().id, action().reviewHash, "2026-09-08T05:01:00.000Z");
  ...
  test("allows distinct account operations in one bundle but reserves proven execution identities", async () => {
    const store = new MemoryMoneyActionStore();
    const first = action();
  ...
  test("keeps terminal status and submission references atomic and unique", async () => {
    const store = new MemoryMoneyActionStore();
    await store.issue(action());
    await store.claim(OWNER, action().id, action().reviewHash, "2026-09-08T05:01:00.000Z");
    const userOperationHash = `0x${"d".repeat(64)}` as const;
