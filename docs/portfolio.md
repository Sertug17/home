...
Direct holdings (ETH + allowlisted ERC-20s) are read from CDP Onchain Data Token Balances (`GET /platform/v2/data/evm/token-balances/base/{address}`) for the session-verified smart account, authenticated with the same server CDP API-key JWT family as other CDP server reads. Home registry decimals and symbols are used; provider metadata is ignored. An allowlisted **non-cash** token omitted from a **complete** page set is a ready `0`. Omitted **cash** (USDC / EURC / IDRX) is verified with `latest` `balanceOf` singles on a dedicated timeout **before** Morpho vault RPC — not leftover Token Balances + vault time, not overlapping vault batches (public Base `-32016`s cash singles while vaults are in flight), not the vault pin block, and not a JSON-RPC batch. Retried once after a short delay: RPC `0` stays ready `0`, a nonzero amount is used, and a failed read is `unavailable` — never an invented zero. Transport, auth, or upstream Token Balances failure still runs cash verify; a later-page Token Balances 429 keeps already-listed directs. If the page budget ends while a cursor remains and the allowlist is not yet satisfied, unseen non-cash tokens are `unavailable` rather than invented zeros. The previous public-RPC batch reader remains in `valuation-rpc.ts` but is no longer the valuation hot path. Phase B/C (materialized store, webhooks, live updates) are locked in [balances inventory](balances-inventory-architecture.md); research detail on [#76](https://github.com/jessepollak/home/issues/76#issuecomment-5594452047). Do not treat this document as the live-update design.
...
When a cash quantity is known but its fiat valuation is unpriced, Home keeps the formatted native token amount and unit in its presentation/cache data but shows only the numeric quantity in the primary Balance row—for example, visible `2,500.00` with accessible detail `2,500.00 IDRX`, never an implied `Rp 2,500.00`. A successfully read quantity uses the normal foreground; failed reads remain `Unavailable`, and unknown quantities remain a muted dash. The display-layer fallback also corrects legacy cached rows without changing their stored DTO, manufacturing a quote, changing valuation status, or adding unpriced holdings to the aggregate.
