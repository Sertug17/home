...
- Invest: a short verified list of Coinbase tokenized stocks on Base. Asset details and trade preview; enable execution only after eligibility and the exact route are verified.
...
- Trading: one quote/execute adapter, initially evaluate CDP Trade API for supported Base routes. Verify exact stock and meme contracts, liquidity, token restrictions and smart-account compatibility. Show expiry, fees, slippage, minimum received and impact before signing.
