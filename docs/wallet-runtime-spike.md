...
- `GET /api/operations` and allowlisted alias `GET /api/actions/operations`: return recent operations for Activity composition, scoped to the full verified owner/provider/address/chain tuple. The ordinary (unscoped) list is a display filter: it omits pre-chain `rejected` / `expired` / failed-without-execution-reference rows and keeps confirmed success plus failed-onchain. It does not delete `MoneyActionStore` records. `scope=unresolved-send` is unchanged.
